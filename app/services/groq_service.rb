class GroqService
  def initialize
    ...
    raise 'GROQ_API_KEY not found in environment variables' unless @api_key
  ...
  def generate_task_creation_response(request_text, context)
    ...
    begin
      ...
      Rails.logger.info "Groq task creation response: #{response.inspect}"
      
      if response && response['content']
        content = response['content']
        parse_task_creation_response(content)
      else
        Rails.logger.error "Unexpected Groq response format: #{response.inspect}"
        { error: "Unexpected response format from Groq API" }
      end
    rescue => e
      Rails.logger.error "Groq service error for task creation: #{e.message}"
